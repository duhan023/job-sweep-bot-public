...
def passes_filters(job: Job, cfg: dict, forbidden: list[str]) -> tuple[bool, str]:
    ...
    for term in targeting.get("exclusions", []):
        ...
        if not term or term in NOOP_EXCLUSIONS or len(term) < 2:
            continue  # "nothing" / "none" means no deal-breakers, not a substring
        if term in blob:
    ...
    window = int(cfg["sources"].get("posted_within_days", 30))
